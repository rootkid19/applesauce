...
usage() {
  cat >&2 <<'EOF'
usage:
  diff_packet002_binary_truth.sh <baseline-label> <patched-label>

example:
  tools/diff_packet002_binary_truth.sh 26.4 26.5

Compares Packet 002 collection outputs and writes a deterministic byte-level
matrix under artifacts/packet002-accounts-privacy/diff-<old>-vs-<new>.
EOF
  exit 2
}
...
BASE_ROOT="$ARTIFACTS/packet002-accounts-privacy/$BASE_LABEL"
PATCH_ROOT="$ARTIFACTS/packet002-accounts-privacy/$PATCH_LABEL"
BASE_DYLD="$ARTIFACTS/dyld-members-packet002/$BASE_LABEL/selected"
PATCH_DYLD="$ARTIFACTS/dyld-members-packet002/$PATCH_LABEL/selected"
OUT="$ARTIFACTS/packet002-accounts-privacy/diff-$BASE_LABEL-vs-$PATCH_LABEL"
...
{
  echo "# Packet 002 Binary Truth Summary"
  echo
  echo "Baseline: $BASE_LABEL"
  echo "Patched: $PATCH_LABEL"
  echo
